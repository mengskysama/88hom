...
function checkUserPhone(){
    ...
    var mobile = jQuery.trim($('#txtUserPhone').val());
	var ddlPhoneType = $("#ddlPhoneType").val();
	if(ddlPhoneType == "1") {
        if(mobile.length > 11) {
            ShowWrong($("#txtUserPhone"), "请填写真实的手机号码", "");
            isTxtUserPhoneValid = false;
            return ;
        }
        if(!(/^13[0-9]{9}|15[0-9][0-9]{8}|18[0-9][0-9]{8}|147[0-9]{8}$/).test(mobile)) {
            ShowWrong($("#txtUserPhone"), "请填写真实的手机号码", "");
            isTxtUserPhoneValid = false;
            return ;
        }
    ...
    if(ddlPhoneType == "2") {
        ...
        marray = mobile.split('-');
        if (mobile.indexOf('-') < 3 || mobile.indexOf('-') > 4) {
            ShowWrong($("#txtUserPhone"), "小灵通电话格式010-59306515", "");
            isTxtUserPhoneValid = false;
            return ;
        }
        if (marray[1].length < 7 || marray[1].length > 8) {
            ShowWrong($("#txtUserPhone"), "小灵通电话格式010-59306515", "");
            isTxtUserPhoneValid = false;
            return ;
        }
        var reg = /^[0-9]*$/;
        if (!reg.test(marray[0])) {
            ShowWrong($("#txtUserPhone"), "小灵通电话格式010-59306515", "");
            isTxtUserPhoneValid = false;
            return ;
        }
        if (!reg.test(marray[1])) {
            ShowWrong($("#txtUserPhone"), "小灵通电话格式010-59306515", "");
            isTxtUserPhoneValid = false;
            return ;
        }
    }
    isTxtUserPhoneValid = true;
}
...
function ajax_mobile() {
    ...
    jQuery.ajax({
        ...
        data: { "userPhone": escape(val(($("#txtUserPhone")))), "num": Math.random().toString() },
        success: function(req) {
            var resu = req.split("|");
            if (resu[0] != "200") {
            	isTxtUserPhoneValid = false;
                ShowWrong($("#txtUserPhone"), "该移动电话号码已被用户绑定", "");
                return false;
            } else {
            	isTxtUserPhoneValid = true;
            }
