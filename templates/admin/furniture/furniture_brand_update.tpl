...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//初始化图片
		var html='<img src="<!--{$cfg.web_url}-->uploads/<!--{$furnitureBrand.picThumb}-->"/>';
		$('#showImg').html(html);
		 $("#file_upload").uploadify({
				...
				'formData'     : {
					...
					'watermarkPos': 9,
					'thumbDir'    : 'furniture/brand/',
					'originalPath': 'furniture/brand/',
...
<div class="title_panel">
	<div class="title">品牌馆添加</div>
</div>
	<form method="post" action="furniture_brand_manager.php?action=doUpdate">
	<input type="hidden" name="id" value="<!--{$furnitureBrand.id}-->"/>
	<input type="hidden" name="pic" value="<!--{$furnitureBrand.pic}-->"/>
	<input type="hidden" name="picThumb" value="<!--{$furnitureBrand.picThumb}-->"/>
	<table cellspacing="0" cellpadding="0" >
		<tr ><td width="100">品牌类别：</td>
		<td width="400">
			<select name="typeId" >
			<!--{foreach from=$cfg.arr_pic.furnitureBrand item=item key=key}--> 
			<!--{if $furnitureBrand.typeId eq key}-->
		...
		<tr ><td width="100">品牌名称：</td><td ><input class="input" type="text" name="name" value="<!--{$furnitureBrand.name}-->"  /></td></tr>
		<tr ><td width="100">品牌链接：</td><td ><input class="input" type="text" name="url" value="<!--{$furnitureBrand.url}-->"  />(可选，格式：http://www.baidu.com)</td></tr>
		<tr ><td width="100">排序号：</td><td><input class="input" type="text" name="orderNum" value="<!--{$furnitureBrand.orderNum}-->"/>(正整数)</td></tr>
		<tr ><td width="100">开启：</td>
		<td>
			<!--{if $furnitureBrand.state eq 1}-->
			<input type="radio" name="state" value="1" checked="checked"/>是 <input type="radio" name="state" value="0"/>否
			<!--{else}-->
			<input type="radio" name="state" value="1" />是 <input checked="checked" type="radio" name="state" value="0"/>否
			<!--{/if}-->
		</td></tr>
