...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('input[value="<!--{$item.infotypeId}-->-<!--{$item.infotypeSubId}-->"]').attr("checked","true");
		<!--{/foreach}-->
...
<div class="title_panel">
	...
	<form method="post" action="info_manager.php?action=doUpdate">
	<input type="hidden" name="infoId" value="<!--{$info.infoId}-->"/>
	<table cellspacing="0" cellpadding="0" >
		<tr><td width="100">标题：</td><td><input maxlength="255" class="input_long" type="text" name="infoTitle" value="<!--{$info.infoTitle}-->"/><font color="red">*</font></td></tr>
		...
		<tr><td width="100">外观图：</td>
			...
			<div style="float:Left;clear:both;">
				<input type="hidden" name="infoPic" value="<!--{$info.infoPic}-->"/>
				<input type="hidden" name="infoPicThumb" value="<!--{$info.infoPicThumb}-->"/>
				<input type="file" name="file_upload" id="file_upload" />
			</div>
					
			</td>
		</tr>
		<tr ><td width="100">信息来源：</td><td><input maxlength="255" class="input_long" type="text" name="infoSource" value="<!--{$info.infoSource}-->"/></td></tr>
		...
		<tr ><td width="100">控制设置：</td>
			<td>
				开启：<!--{if $info.infoState eq 1}-->
						<input type="radio" name="infoState" checked="checked" value="1"/>是  <input type="radio" name="infoState"  value="0"/>否
					 <!--{else}-->
					 	<input type="radio" name="infoState"  value="1"/>是  <input type="radio" name="infoState" checked="checked"  value="0"/>否
					 <!--{/if}-->
				推荐：
					<!--{if $info.infoState eq 1}-->
						<input type="radio" name="infoSuggest" checked="checked" value="1"/>是  <input type="radio" name="infoSuggest"  value="0"/>否 
					 <!--{else}-->
					 	<input type="radio" name="infoSuggest"  value="1"/>是  <input type="radio" name="infoSuggest" checked="checked" value="0"/>否 
					 <!--{/if}-->
				排序号：<input class="input_short" type="text" name="infoLayer" value="<!--{$info.infoLayer}-->"/>(正整数)
			</td>
		</tr>
