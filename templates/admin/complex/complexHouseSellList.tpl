...
<div class="title_panel">
	<div class="title">住宅房源</div>
</div>
<div style="float:left;width:100%;margin:10px 0px;">
	<form action="complexHouseList.php" method="post">
...
<form id="listForm" name="listForm" action="action.php?action=complexHouseChangeStateByIds" method="post">
	...
	<table cellspacing="0" cellpadding="0" >
		...
		<!--{foreach from=$houseList item=item key=key}-->
		<tr align="center">
			<td><input type="checkbox" name="chk[]" value="<!--{$item.houseId}-->"/></td>
			<td><!--{$item.houseId}--></td>
			<td><!--{$item.houseAreaName}--></td>
			...
			<td><!--{$item.houseSellPrice}--></td>
			<td><!--{$item.houseBuildArea}--></td>
			<td><!--{$item.houseUseArea}--></td>
			<td><!--{$item.houseUpdateTime|date_format:'%Y-%m-%d %H:%M:%S'}--></td>
			<td>
			<!--{if $item.houseState==1}-->
			...
			<td>
			<!--{if $item.houseState==1}-->
				<a href="complexHouseDetail.php?id=<!--{$item.houseId}-->">详情</a> 
				<a href="action.php?action=complexHouseChangeStateById&id=<!--{$item.houseId}-->&type=<!--{$type}-->&state=5&s=<!--{$state}-->">通过</a> 
				<a href="action.php?action=complexHouseChangeStateById&id=<!--{$item.houseId}-->&type=<!--{$type}-->&state=4&s=<!--{$state}-->">违规退回</a> 
				<a href="javascript:exeAdminDelMessage('action.php?action=complexHouseChangeStateById&id=<!--{$item.houseId}-->&type=<!--{$type}-->&state=2&s=<!--{$state}-->');">删除</a> 
			<!--{else if $item.houseState==5}-->
				<a href="complexHouseDetail.php?id=<!--{$item.houseId}-->">详情</a> 
				<a href="action.php?action=complexHouseChangeStateById&id=<!--{$item.houseId}-->&type=<!--{$type}-->&state=4&s=<!--{$state}-->">违规退回</a> 
				<a href="javascript:exeAdminDelMessage('action.php?action=complexHouseChangeStateById&id=<!--{$item.houseId}-->&type=<!--{$type}-->&state=2&s=<!--{$state}-->');">删除</a> 
	...
	<div style="float:left;margin-top:5px;">
		...
		<!--{if $item.houseState==1}-->
			<a href="javascript:void(0);" onclick="if(confirm('你确认通过选中的条目？')){$('#listForm').attr('action','action.php?action=complexHouseChangeStateByIds&state=5');$('#listForm').submit();}">通过所选</a> 
			<a href="javascript:void(0);" onclick="if(confirm('你确认退回选中的条目？')){$('#listForm').attr('action','action.php?action=complexHouseChangeStateByIds&state=4');$('#listForm').submit();}">退回所选</a>
			<a href="javascript:void(0);" onclick="if(confirm('你确认删除选中的条目？')){$('#listForm').attr('action','action.php?action=complexHouseChangeStateByIds&state=2');$('#listForm').submit();}">删除所选</a>
		<!--{else if $item.houseState==5}-->
			<a href="javascript:void(0);" onclick="if(confirm('你确认退回选中的条目？')){$('#listForm').attr('action','action.php?action=complexHouseChangeStateByIds&state=4');$('#listForm').submit();}">退回所选</a>
			<a href="javascript:void(0);" onclick="if(confirm('你确认删除选中的条目？')){$('#listForm').attr('action','action.php?action=complexHouseChangeStateByIds&state=2');$('#listForm').submit();}">删除所选</a>
		<!--{else if $item.houseState==4}-->
