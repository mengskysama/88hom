...
<div class="title_panel">
	<div class="title">写字楼房源</div>
</div>
<div style="float:left;width:100%;margin:10px 0px;">
	<form action="complexOfficeList.php" method="post">
...
<form name="listForm" id="listForm" action="action.php?action=complexOfficeChangeStateByIds" method="post">
	...
	<table cellspacing="0" cellpadding="0" >
		...
		<!--{foreach from=$officeList item=item key=key}-->
		<tr align="center">
			<td><input type="checkbox" name="chk[]" value="<!--{$item.officeId}-->"/></td>
			<td><!--{$item.officeId}--></td>
			<td><!--{$item.officeAreaName}--></td>
			<td><!--{$item.officeTitle}--></td>
			<td><!--{$item.officeRentPrice}--></td>
			<td><!--{$item.officeProFee}--></td>
			<td><!--{$item.officeBuildArea}--></td>
			<td><!--{$item.officeUpdateTime|date_format:'%Y-%m-%d %H:%M:%S'}--></td>
			<td>
			<!--{if $item.officeState==1}-->
				<font color="red">发布待审核</font>
			<!--{else if $item.officeState==5}-->
				<font color="blue">审核已通过</font>
			<!--{else if $item.officeState==4}-->
				<font color="blue">违规被退回</font>
			<!--{else if $item.officeState==2}-->
				<font color="blue">发布被删除</font>
			<!--{/if}-->
			</td>
			<td>
			<!--{if $item.officeState==1}-->
				<a href="complexOfficeDetail.php?id=<!--{$item.officeId}-->">详情</a> 
				<a href="action.php?action=complexOfficeChangeStateById&id=<!--{$item.officeId}-->&type=<!--{$type}-->&state=5&s=<!--{$state}-->">通过</a> 
				<a href="action.php?action=complexOfficeChangeStateById&id=<!--{$item.officeId}-->&type=<!--{$type}-->&state=4&s=<!--{$state}-->">违规退回</a> 
				<a href="javascript:exeAdminDelMessage('action.php?action=complexOfficeChangeStateById&id=<!--{$item.officeId}-->&type=<!--{$type}-->&state=2&s=<!--{$state}-->');">删除</a> 
			<!--{else if $item.officeState==5}-->
				<a href="complexOfficeDetail.php?id=<!--{$item.officeId}-->">详情</a> 
				<a href="action.php?action=complexOfficeChangeStateById&id=<!--{$item.officeId}-->&type=<!--{$type}-->&state=4&s=<!--{$state}-->">违规退回</a> 
				<a href="javascript:exeAdminDelMessage('action.php?action=complexOfficeChangeStateById&id=<!--{$item.officeId}-->&type=<!--{$type}-->&state=2&s=<!--{$state}-->');">删除</a> 
	...
	<div style="float:left;margin-top:5px;">
		...
		<!--{if $item.officeState==1}-->
			<a href="javascript:void(0);" onclick="if(confirm('你确认通过选中的条目？')){$('#listForm').attr('action','action.php?action=complexOfficeChangeStateByIds&state=5');$('#listForm').submit();}">通过所选</a> 
			<a href="javascript:void(0);" onclick="if(confirm('你确认退回选中的条目？')){$('#listForm').attr('action','action.php?action=complexOfficeChangeStateByIds&state=4');$('#listForm').submit();}">退回所选</a>
			<a href="javascript:void(0);" onclick="if(confirm('你确认删除选中的条目？')){$('#listForm').attr('action','action.php?action=complexOfficeChangeStateByIds&state=2');$('#listForm').submit();}">删除所选</a>
		<!--{else if $item.officeState==5}-->
			<a href="javascript:void(0);" onclick="if(confirm('你确认退回选中的条目？')){$('#listForm').attr('action','action.php?action=complexOfficeChangeStateByIds&state=4');$('#listForm').submit();}">退回所选</a>
			<a href="javascript:void(0);" onclick="if(confirm('你确认删除选中的条目？')){$('#listForm').attr('action','action.php?action=complexOfficeChangeStateByIds&state=2');$('#listForm').submit();}">删除所选</a>
		<!--{else if $item.officeState==4}-->
