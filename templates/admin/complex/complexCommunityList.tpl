...
<div class="title_panel">
	<div class="title">新增小区(外部添加)</div>
</div>
<div style="float:left;width:100%;margin:10px 0px;">
	<form action="complexCommunityList.php" method="post">
...
<form name="listForm" action="action.php?action=complexCommunityChangeStateByIds" method="post">
	<table cellspacing="0" cellpadding="0" >
		...
		<tr align="center">
			<td><input type="checkbox" name="chk[]" value="<!--{$item.communityId}-->"/></td>
			<td><!--{$item.communityId}--></td>
			<td><!--{$item.communityName}--></td>
			<td><!--{$item.communityAreaName}--></td>
			<td>
			<!--{if $item.communityIsHouseType==1}-->
				住宅
			<!--{/if}--> 
			<!--{if $item.communityIsBusinessType==1}-->
				商铺
			<!--{/if}--> 
			<!--{if $item.communityIsOfficeType==1}-->
				写字楼
			<!--{/if}-->
			<!--{if $item.communityIsVillaType==1}-->
				别墅
			<!--{/if}-->
			</td>
			<td><!--{$item.communityAddress}--></td>
			<td><!--{$item.communityTraffic}--></td>
			<td><!--{$item.communityPeriInfo}--></td>
			<td><!--{$item.communityCreateTime|date_format:'%Y-%m-%d %H:%M:%S'}--></td>
			<td><!--{if $item.communityState==0}--><font color="red">待审</font><!--{/if}--></td>
			<td><a href="complexCommunityModify.php?id=<!--{$item.communityId}-->">去审核</a> <a href="javascript:exeAdminDelMessage('action.php?action=complexCommunityChangeStateById&id=<!--{$item.communityId}-->');">删除</a> </td>
		</tr>
		<!--{/foreach}-->
	</table>
	<!--{if $communityList neq null}-->
