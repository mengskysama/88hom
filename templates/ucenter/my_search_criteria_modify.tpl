...
<title>修改我的<!--{if $cType==1}-->找房<!--{elseif $cType==2}-->租房<!--{/if}-->条件_用户中心_房不剩房</title>
<!--{$jsFiles}-->
<!--{$cssFiles}-->
</head>
<body>
...
<!--{include file="$ucenter_user_left_menu"}-->
    <div class="qg_r">
    	<p>你的位置： <a href="ucenter_user.php">我的房不剩房<a> </a> > <a href="user_wanted.php?wType=<!--{$cType}-->">我<!--{if $cType==1}-->要买<!--{elseif $cType==2}-->租房<!--{/if}-->房</a> > <a href="user_search_criteria.php?cType=<!--{$cType}-->">我的<!--{if $cType==1}-->找房<!--{elseif $cType==2}-->租房<!--{/if}-->条件</a> > <a href="javascript:void(0);"></a>修改我的<!--{if $cType==1}-->找房<!--{elseif $cType==2}-->租房<!--{/if}-->条件</p>
        <div class="qgxq2">
       	  <p class="wyqg1"><b class="z14">基本资料</b><font class="red">*</font>为必填项</p>
       	  <div class="other" id="div_err_msg">  	  	 
       	  </div>
       	   <form id="do_search_criteria" action="do_search_criteria.php?action=modify" method="post">
       	   	<input type="hidden" id="id" name="id" value="<!--{$infoDetail.id}-->"/>
       	    <input type="hidden" id="userId" name="userId" value="<!--{$userId}-->"/>
       	    <input type="hidden" id="cType" name="cType" value="<!--{$infoDetail.c_type}-->" />
       	    <input type="hidden" id="hand" name="hand" value="<!--{$infoDetail.hand}-->" />
              <table width="680" height="351" border="0" cellpadding="0" cellspacing="0" class="mt10">
                  <tr>
                      <td>房屋类型：</td>
                      <td>&nbsp;<select name="houseType" id="houseType">
                      <!--{html_options options=$infoTypeSelectList selected=$infoDetail.house_type}-->
                      </select></td>      
                  </tr>
                  <tr>
                    <td>售价：</td>
                      <td class="grzc_33 p5"><input id="price" name="price" type="text"  value="<!--{$infoDetail.price}-->" />(元/平米)</td>
                  </tr>
                  <tr>
			...
			<table id="areaTable" style="width: 360px;"  border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <th style="width: 360px;" align="left">
                        ...
                        <div style="position: relative; z-index: 10000;" >
                            <input type="text" id="txtCity" autocomplete="off" class="input_cs" style="width: 60px;" name="txtCity" 
                                value="<!--{$infoDetail.city}-->" onkeyup="SelectCity(this.value);onKey(event,'search_select');" onkeydown="SelectonKeyDown('txtCity',event,'search_select')"
                            ...
                            <input type="text" id="txtDistrict" autocomplete="off" name="txtDistrict" readonly="readonly" value="<!--{$infoDetail.district}-->"
                            ...
                            <input type="text" class="input_cs" autocomplete="off" id="txtComareas" name="txtComareas" readonly="readonly"
                                value="<!--{$infoDetail.comareas}-->" onclick="ShowComareas();" onkeyup="onKey(event,'search_c');"
                             ...
                             <!--省，市，区域，片区下标,以"-"隔开 -->
							<input type="hidden" name="areaIndex"	id="areaIndex" value="<!--{$infoDetail.area_index}-->"/>	
                  ...
                  <tr>
                      <td>名称：</td>
                      <td class="grzc_33 p5"><input id="area" name="area" type="text"  value="<!--{$infoDetail.area}-->" /></td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                      <td>&nbsp;</td>
                      <td colspan="3"><input name="button2" type="submit" class="denglu1 l" id="button2" value="修改" /></td>
                  </tr>
              </table>
             </form>
        	</div>
