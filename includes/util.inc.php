...
function sysSearchPageInfo($totalNum=0,$pageSize=10,$currentPage=null,$url,$params){
		//当前页 
		$currentPage=($currentPage == null ? 1 : $currentPage);
		//echo $totalNum.'-'.$pageSize.'-'.$currentPage.'-'.$url;
		//总页数
		...
		//页信息
		$pageInfo = '<label class="pagingInfo">'.$currentPage.'/'.$totalPage.',&nbsp;'.$pageSize.'/页,&nbsp;&nbsp;共:&nbsp;'.$totalNum
		.'&nbsp;&nbsp;</label><a class="pointer" href="'.$url.'_1_'.$params.'.htm" title="首页"><<</a>';
		if($pageParagraph > 1)
			$pageInfo .= '<a class="pointer" href="'.$url.'_'.(($pageParagraph - 1) * 10).'_'.$params.'.htm" title="前十页"><&nbsp;</a>';
		...
		for($i=$startPage;$i<$endPage;$i++){
			if($i == $currentPage)
			{
				...
				if($i <= $totalPage){
					$pageInfo .= '<a href="'.$url.'_'.$i.'_'.$params.'.htm">'.$i.'</a>';
				}
			}
		}
		if($pageParagraph < $maxPageParagraph)
		{
			$pageInfo .= '<a class="pointer" href="'.$url.'_'.($pageParagraph *10+1).'_'.$params.'.htm" title="下十页">&nbsp;></a>';
		}
		$pageInfo .= '<a class="pointer" href="'.$url.'_'.$totalPage.'_'.$params.'.htm" title="末页">>></a>';
		return $pageInfo;
}
//added by Cheneil
