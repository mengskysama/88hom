...
$tpl_name=$tpl_dir.'complexShopsSellList.tpl';
$html->title='商铺房源列表';
...
if($type==1){
	...
	$tpl_name=$tpl_dir.'complexShopsRentList.tpl';
}
...
$fileName='complexShopsList.php?type='.$type.'&state='.$state.'&search='.$search.'&areaIndex='.$areaIndex.'&page';
...
$shopsList=$auditService->getShopsList($auditModel);
$shopsCount=$auditService->countShops($auditModel);
//
$divPage='';
if(!empty($shopsCount)&&$shopsCount['counts']>0){
	$divPage=sysAdminPageInfo($shopsCount['counts'],$step,$page,$fileName,'');
}else{
	$divPage='';
}
...
$smarty->assign('shopsList',$shopsList);
