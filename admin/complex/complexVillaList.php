...
$tpl_name=$tpl_dir.'complexVillaSellList.tpl';
$html->title='别墅房源列表';
...
if($type==1){
	...
	$tpl_name=$tpl_dir.'complexVillaRentList.tpl';
}
...
$fileName='complexVillaList.php?type='.$type.'&state='.$state.'&search='.$search.'&areaIndex='.$areaIndex.'&page';
...
$villaList=$auditService->getVillaList($auditModel);
$villaCount=$auditService->countVilla($auditModel);
//
$divPage='';
if(!empty($villaCount)&&$villaCount['counts']>0){
	$divPage=sysAdminPageInfo($villaCount['counts'],$step,$page,$fileName,'');
}else{
	$divPage='';
}
...
$smarty->assign('villaList',$villaList);
