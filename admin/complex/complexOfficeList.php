...
$tpl_name=$tpl_dir.'complexOfficeSellList.tpl';
$html->title='写字楼房源列表';
...
if($type==1){
	...
	$tpl_name=$tpl_dir.'complexOfficeRentList.tpl';
}
...
$fileName='complexOfficeList.php?type='.$type.'&state='.$state.'&search='.$search.'&areaIndex='.$areaIndex.'&page';
...
$officeList=$auditService->getOfficeList($auditModel);
$officeCount=$auditService->countOffice($auditModel);
//
$divPage='';
if(!empty($officeCount)&&$officeCount['counts']>0){
	$divPage=sysAdminPageInfo($officeCount['counts'],$step,$page,$fileName,'');
}else{
	$divPage='';
}
...
$smarty->assign('officeList',$officeList);
