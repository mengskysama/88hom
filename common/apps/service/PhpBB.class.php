...
class PHPBB {
	...
	private $dbuser="root";
	private $passwd="";
	private $sqlname="phpbb";
	private $bbsPath="E:/home/web/88hom/bbs/";
	
	public function __construct() {
		$db = mysql_connect($this->hostname, $this->dbuser, $this->passwd);
		mysql_select_db ( $this->sqlname, $db );
	}
	/**
	 * 同步登陆论坛
	 */
	...
	function forumRegister($user_name, $password, $email, $coppa = false) {
      ...
      WHERE group_name = '" . $db->sql_escape ( $group_name ) . "'
		...
		if ($user_id) {
			$this->forumLogin( $data [username], $data [new_password], false );
		}
	...
	function forumEdit($user_id, $email, $password, $cu_password = "") {
		...
		if (! $cu_password) {
			...
			$sql_ary1 = array ('user_email' => $email, 'user_email_hash' => crc32 ( $email ) . strlen ( $email ) );
			
			if ($password) {
				$sql_ary2 = array ('user_password' => phpbb_hash ( $password ), 'user_passchg' => time () );
			}
			...
			if (! $user->data ['user_password']) {
				$this->forumLogin ( $user_id, $cu_password, false );
				$user->session_begin ();
				$auth->acl ( $user->data );
				$user->setup ();
			}
			...
			if ($user->data ['user_password']) {
				...
				$data = array ('email' => strtolower ( request_var ( 'email', $user->data ['user_email'] ) ), 'email_confirm' => strtolower ( request_var ( 'email_confirm', '' ) ), 'new_password' => request_var ( 'new_password', '', true ), 'password_confirm' => request_var ( 'password_confirm', '', true ) );
				
				$sql_ary = array ('user_email' => ($auth->acl_get ( 'u_chgemail' )) ? $data ['email'] : $user->data ['user_email'], 'user_email_hash' => ($auth->acl_get ( 'u_chgemail' )) ? crc32 ( $data ['email'] ) . strlen ( $data ['email'] ) : $user->data ['user_email_hash'], 'user_password' => ($auth->acl_get ( 'u_chgpasswd' ) && $data ['new_password']) ? phpbb_hash ( $data ['new_password'] ) : $user->data ['user_password'], 'user_passchg' => ($auth->acl_get ( 'u_chgpasswd' ) && $data ['new_password']) ? time () : 0 );
